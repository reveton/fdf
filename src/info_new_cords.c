...
void    info(t_fdf *fdf)
{
    ...
    mlx_string_put(fdf->mlx, fdf->win, 80, 120, 0xFFFF00, "ARROW LEFT,RIGHT,UP,DOWN");
...
void    generate_new_cords(t_fdf *cord)
{
    ...
    while (count_c)
    {
        cord->cord[i]->x_coords = (cord->cord[i]->x_coords - 1) - cord->count_x / 2;
        cord->cord[i]->y_coords = (cord->cord[i]->y_coords - 1) - cord->count_y / 2;
        count_c--;
        i++;
    }
}